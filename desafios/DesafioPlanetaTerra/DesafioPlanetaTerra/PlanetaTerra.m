...
static NSMutableDictionary *_cacheBandeira;

+(UIImage *) bandeiraPaisCache:(NSString*)url {
    if (!_cacheBandeira)
        _cacheBandeira = [NSMutableDictionary dictionary];
    
    UIImage *cache = [_cacheBandeira objectForKey:url];
    
    if (!cache) {
        NSURL *bandeiraURL = [NSURL URLWithString:url];
        ...
        [_cacheBandeira setObject:cache forKey:url];
    }
...
+(UIImage *)bandeiraPais:(NSString *)nomePais
    ...
    return [self bandeiraPaisCache:bandeiraURLString];
}
